...
function getCredentials() {
  const credentials = JSON.parse(JSON.stringify(config.gcs.credentials));
  if (config.livesIn === "GCP") delete credentials.keyFilename;

  return credentials;
}

function createWriteStream(gcsPath, writeStreamOptions) {
...
function createReadStream(gcsPath, readStreamOptions) {
  const {Bucket, Key} = parseUri(gcsPath);
