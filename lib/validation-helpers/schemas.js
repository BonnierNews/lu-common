...
const distributionFeeSchema = joi.object().keys({
  amount: joi.number().precision(0).positive().allow(0).required(),
  startDate: joi.string().when("monthlyDelay", {is: joi.exist(), then: joi.forbidden(), otherwise: joi.optional()}),
  monthlyDelay: joi.number().precision(0).allow(0)
});

module.exports = {
  addressSchema,
  distributionFeeSchema
};
