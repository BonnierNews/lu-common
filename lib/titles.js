...
function getTsCode(namespace, title) {
  assert(namespace, "No namespace was provided");
  if (!validNamespaces.includes(namespace)) return;

  if (!title && [ "expressen", "dn" ].includes(namespace)) {
    title = namespace;
  }
  let usedTitle = title;
  if (!usedTitle) {
    let defaultTitle;
    if (productConfig[namespace].length === 1) defaultTitle = productConfig[namespace][0].title; // only 1 title for the namespace, so use it
    assert(defaultTitle, "no title was provided for namespace with multiple titles");
    usedTitle = defaultTitle;
  }
  const product = productMapping.find(
    (p) =>
      p.namespace === namespace &&
      (p.title === usedTitle || (p.alternativeTitles && p.alternativeTitles.includes(usedTitle)))
  );

  if (product) {
    const tsCode = product.tsCode;
    return tsCode;
  }
  return;
}
